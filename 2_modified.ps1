...
# Prompt user to specify if the machine is a Laptop or PC
$laptopOrPC = Read-Host "Is this a Laptop or PC? (Enter 'L' or 'P')"

# Get the serial number of the machine
$serial = Get-WmiObject win32_bios | Select-Object -ExpandProperty SerialNumber

# Determine the prefix based on the user's input
if ($laptopOrPC -eq "L") {
    $prefix = "BLW-"
} elseif ($laptopOrPC -eq "P") {
    $prefix = "BDW-"
} else {
    Write-Host "Invalid input. Please enter 'Laptop' or 'PC'."
    exit
}

# Generate the new computer name
$newname = $prefix + $serial.Substring($serial.Length - 7)

# Change the computer name
Rename-Computer -NewName $newname
Write-Host "The computer has been renamed to $newname."

Configuration superbank_cis_windows10_v3 {
    param (
    ...
    Push-Location
    Set-Location HKLM:
    if (-not (Test-Path .\SOFTWARE\Policies\Microsoft\Windows\RemovableStorageDevices)) {
        New-Item -Path .\SOFTWARE\Policies\Microsoft\Windows -Name RemovableStorageDevices
    }
    Pop-Location
    ...
    Push-Location
    Set-Location HKLM:
    if (-not (Test-Path ".\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System")) {
    ...
    Push-Location
    Set-Location HKLM:
    if (-not (Test-Path ".\Microsoft\Windows\CurrentVersion\Policies\Explorer")) {
        New-Item -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Explorer"
    }
    New-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" -Name "HideSCAMeetNow" -Value "1" -PropertyType "Dword"
    Pop-Location

    # Disable Bluetooth
    Push-Location
    Set-Location HKLM:
    if (-not (Test-Path ".\SYSTEM\CurrentControlSet\services\BTHPORT\Parameters")) {
        New-Item -Path "HKLM:\SYSTEM\CurrentControlSet\services\BTHPORT\Parameters"
    }
    New-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\services\BTHPORT\Parameters" -Name "DisableFsquirt" -Value "1" -PropertyType "Dword"
    Fsquirt.exe -UnRegister
    Pop-Location

    # Disable News
    Push-Location
    Set-Location HKLM:
    if (-not (Test-Path ".\SOFTWARE\Policies\Microsoft\Windows\Windows Feeds")) {
        New-Item -Path ".\SOFTWARE\Policies\Microsoft\Windows" -Name "Windows Feeds"
    }
    New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\Windows Feeds" -Name "EnableFeeds" -Value "0" -PropertyType "Dword"
    Pop-Location

    # Disable news widget on Windows Search
    Push-Location
    Set-Location HKLM:
    if (-not (Test-Path ".\SOFTWARE\Policies\Microsoft\Windows\Windows Search")) {
        New-Item -Path ".\SOFTWARE\Policies\Microsoft\Windows" -Name "Windows Search"
    }
    New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\Windows Search" -Name "EnableDynamicContentInWSB" -Value "0" -PropertyType "Dword"
    Pop-Location
Start-Sleep 5
    # Disable username logon
    Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" -Name "dontdisplaylastusername" -Value "1"

    # Disable Microsoft account and sign-in options
    ...
    # Disable password view
    Push-Location
    Set-Location HKLM:
    if (-not (Test-Path ".\SOFTWARE\Policies\Microsoft\Windows\CredUI")) {
        New-Item -Path ".\SOFTWARE\Policies\Microsoft\Windows" -Name "CredUI"
    }
...
Start-Sleep 5
    ...
    Push-Location
    Set-Location HKCU:
    if (-not (Test-Path ".\SOFTWARE\Microsoft\Windows\CurrentVersion")) {
        New-Item -Path ".\SOFTWARE\Microsoft\Windows" -Name "CurrentVersion"
    }
    if (-not (Test-Path ".\SOFTWARE\Microsoft\Windows\CurrentVersion\UserProfileEngagement")) {
        New-Item -Path ".\SOFTWARE\Microsoft\Windows\CurrentVersion" -Name "UserProfileEngagement"
    }
    New-ItemProperty -Path "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\UserProfileEngagement" -Name "ScoobeSystemSettingsEnabled" -Value "0" -PropertyType "Dword"
    New-ItemProperty -Path "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\ContentDeliveryManager" -Name "SubscribedContent-310093Enabled" -Value "0" -PropertyType "Dword"
...
Start-Sleep 5
superbank_cis_windows10_v3
Start-DscConfiguration -Path .\superbank_cis_windows10_v3 -Force -Wait
Set-NetConnectionProfile -NetworkCategory Public
## Patch Management Part ###
...
Start-Sleep 5
# Loop to uninstall the apps

foreach ($app in $apps) {
    Write-Output "Removing $app"

    # Try to remove the app for all users if it exists
    $appVersion = (Get-AppxPackage -Name $app).Version
    if ($appVersion) {
        Get-AppxPackage -Name $app -AllUsers | Remove-AppxPackage -AllUsers
    }

    # Remove the app from the Windows image to prevent reinstall
...
# Add local accounts
Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System" -Name "dontdisplaylastusername" -Value "1"
NET USER bts-support "Am3r1c4n0%" /ADD
Set-LocalUser -Name "bts-support" -PasswordNeverExpires:$true -UserMayChangePassword:$false
Add-LocalGroupMember -Group "Administrators" -Member "bts-support"
Start-Sleep 5
NET USER bts-tshoot "John?Snow1" /ADD
Set-LocalUser -Name "bts-tshoot" -PasswordNeverExpires:$true -UserMayChangePassword:$false
Start-Sleep 5
# Remove other local users
Remove-LocalUser -Name "fama-tshoot"
Remove-LocalUser -Name "fama-support"
...
# Remove OneDrive
echo "Stopping OneDrive and Explorer processes"
...
echo "Disabling OneDrive via Group Policies"
...
echo "Removing OneDrive from startup for new users"
reg load "hku\Default" "C:\Users\Default\NTUSER.DAT"
reg delete "HKEY_USERS\Default\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" /v "OneDriveSetup" /f
reg unload "hku\Default"

echo "Removing OneDrive start menu entry"
Remove-Item -Force -ErrorAction SilentlyContinue "$env:userprofile\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\OneDrive.lnk"

echo "Restarting Explorer"
Start-Process "explorer.exe"

echo "Waiting for Explorer to reload..."
Start-Sleep 15
...
Get-ChildItem "$env:WinDir\WinSxS\*onedrive*" | ForEach-Object {
    Takeown-Folder $_.FullName
